...
namespace WaveMSS
{
    ...
    public class WaveMediaStreamSource : MediaStreamSource, IDisposable
    {
        ...
        public WaveMediaStreamSource(Stream stream)
        {
		  ...
		  public WaveMediaStreamSource(Stream stream,double _speed)
		  {
			  this.stream = stream;
			  fSpeed = _speed;
		  }
        ...
        {
	            // Create a parser
            this.wavParser = new WavParser(this.stream,fSpeed);

            // Parse the header
            this.wavParser.ParseWaveHeader();
				this.wavParser.WaveFormatEx.SamplesPerSec=(int)(this.wavParser.WaveFormatEx.SamplesPerSec * fSpeed);
				this.wavParser.WaveFormatEx.AvgBytesPerSec = (int)(this.wavParser.WaveFormatEx.AvgBytesPerSec * fSpeed);
